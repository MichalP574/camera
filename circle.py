...
def reorderFiducialMarkers(fiducialMarkers):
    ...
    # program to compute sum of all elements, sum of each column(0) and sum of each row(1) of a given array
...
def getCircle(img, minRadius=10, maxRadius=40,):
    imgC = img.copy()
    circles = cv2.HoughCircles(imgC, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=30, minRadius=minRadius,
                               maxRadius=maxRadius)
    ...
    if circles is not None:
        ...
        for (x, y, r) in circles:
            cv2.circle(imgC, (x, y), r, (255, 0, 0), 20)
    return imgC
